...
// Gargoyle sentinel SVG — detailed with wings, horns, glowing eyes, claws
function GargoyleSentinel({ side = 'left' }) {
  const flip = side === 'right'
  return (
    <svg
      className="gargoyle-sentinel"
      viewBox="0 0 120 180"
    ...
    >
      <g fill="rgba(30,20,55,0.9)" stroke="rgba(120,70,220,0.4)" strokeWidth="0.8">
        ...
        {/* Spine ridges */}
        <path d="M55,62 L60,58 L65,62" strokeWidth="1" fill="none" stroke="rgba(100,60,180,0.3)" />
        <path d="M52,72 L60,68 L68,72" strokeWidth="1" fill="none" stroke="rgba(100,60,180,0.3)" />
      </g>
    </svg>
  )
}

// Enhanced Castle silhouette — massive, 7+ towers, viewBox 1200 wide
function EnhancedCastleSilhouette() {
  return (
    <svg
      className="castle-silhouette"
    ...
    >
      ...
      {/* Left wall with battlements */}
      <rect x="50" y="155" width="130" height="30" fill="rgba(8,3,10,0.97)" />
      {[0,18,36,54,72,90,108].map(x => (
        <rect key={`lw${x}`} x={52+x} y="143" width="12" height="14" fill="rgba(8,3,10,0.97)" />
      ))}

      {/* Flying buttress left */}
      <path d="M170,115 Q145,90 120,155" stroke="rgba(8,3,10,0.97)" strokeWidth="6" fill="none" />
      <path d="M170,105 Q150,85 130,150" stroke="rgba(6,2,8,0.98)" strokeWidth="3" fill="none" />

      {/* ── Tower 3: Left major tower ── */}
      ...
      <rect x="195" y="95" width="12" height="6" fill="rgba(80,30,160,0.45)" />
      {/* Window */}
      <rect x="220" y="90" width="5" height="18" fill="rgba(60,20,120,0.35)" />

      {/* ── Tower 4: Left-center narrow tower ── */}
      <rect x="275" y="85" width="35" height="100" fill="rgba(7,2,9,0.98)" />
      <polygon points="275,85 292,50 310,85" fill="rgba(7,2,9,0.98)" />
      <polygon points="288,58 292,30 296,58" fill="rgba(7,2,9,0.98)" />

      {/* Center walkway left */}
      <rect x="248" y="135" width="170" height="50" fill="rgba(7,2,9,0.97)" />
      {[0,24,48,72,96,120,144].map(x => (
        <rect key={`cw${x}`} x={250+x} y="122" width="16" height="16" fill="rgba(7,2,9,0.97)" />
      ))}

      {/* Flying buttress to main tower left */}
      <path d="M418,80 Q380,55 340,130" stroke="rgba(7,2,9,0.97)" strokeWidth="6" fill="none" />

      {/* ── Tower 5: GRAND CENTRAL KEEP ── */}
      ...
      {/* Rose window — elaborate circle with cross pattern */}
      ...
      <line x1="448" y1="25" x2="482" y2="59" stroke="rgba(139,92,246,0.1)" strokeWidth="0.8" />
      <line x1="482" y1="25" x2="448" y2="59" stroke="rgba(139,92,246,0.1)" strokeWidth="0.8" />
      {/* Rose window glow */}
      <circle cx="465" cy="42" r="24" fill="rgba(139,92,246,0.04)" />

      {/* Grand arched gateway — glowing purple */}
      ...
      {/* Gateway glow on ground */}
      <ellipse cx="465" cy="183" rx="20" ry="8" fill="rgba(139,92,246,0.12)" />

      {/* Main tower windows */}
      <rect x="435" y="72" width="6" height="22" fill="rgba(139,92,246,0.12)" />
      <rect x="488" y="72" width="6" height="22" fill="rgba(139,92,246,0.1)" />
      <rect x="435" y="140" width="5" height="15" fill="rgba(80,30,160,0.3)" />
      <rect x="490" y="140" width="5" height="15" fill="rgba(80,30,160,0.3)" />

      {/* Flying buttress to main tower right */}
      <path d="M510,80 Q550,55 590,130" stroke="rgba(7,2,9,0.97)" strokeWidth="6" fill="none" />

      {/* Center walkway right */}
      <rect x="520" y="135" width="170" height="50" fill="rgba(7,2,9,0.97)" />
      {[0,24,48,72,96,120,144].map(x => (
        <rect key={`rw${x}`} x={522+x} y="122" width="16" height="16" fill="rgba(7,2,9,0.97)" />
      ))}

      {/* ── Tower 6: Right-center narrow tower ── */}
      <rect x="620" y="85" width="35" height="100" fill="rgba(7,2,9,0.98)" />
      <polygon points="620,85 637,50 655,85" fill="rgba(7,2,9,0.98)" />
      <polygon points="633,58 637,30 641,58" fill="rgba(7,2,9,0.98)" />

      {/* ── Tower 7: Right major tower ── */}
      ...
      <rect x="713" y="95" width="12" height="6" fill="rgba(80,30,160,0.45)" />
      <rect x="695" y="90" width="5" height="18" fill="rgba(60,20,120,0.35)" />

      {/* Flying buttress right */}
      <path d="M760,115 Q785,90 810,155" stroke="rgba(8,3,10,0.97)" strokeWidth="6" fill="none" />
      <path d="M760,105 Q780,85 800,150" stroke="rgba(6,2,8,0.98)" strokeWidth="3" fill="none" />

      {/* Right wall with battlements */}
      <rect x="750" y="155" width="140" height="30" fill="rgba(8,3,10,0.97)" />
      {[0,18,36,54,72,90,108,126].map(x => (
        <rect key={`rwl${x}`} x={752+x} y="143" width="12" height="14" fill="rgba(8,3,10,0.97)" />
      ))}

      {/* ── Tower 8: Right turret ── */}
      <rect x="900" y="105" width="40" height="80" fill="rgba(8,3,10,0.97)" />
      <polygon points="900,105 920,60 940,105" fill="rgba(8,3,10,0.97)" />
      <polygon points="914,70 920,40 926,70" fill="rgba(8,3,10,0.97)" />
      <rect x="916" y="125" width="4" height="14" fill="rgba(80,30,160,0.35)" />

      {/* ── Tower 9: Far right watchtower ── */}
      <rect x="960" y="110" width="45" height="75" fill="rgba(8,3,10,0.97)" />
      <polygon points="960,110 982,65 1005,110" fill="rgba(8,3,10,0.97)" />
      <polygon points="976,74 982,42 988,74" fill="rgba(8,3,10,0.97)" />

      {/* Far right wall extension */}
      <rect x="1005" y="155" width="100" height="30" fill="rgba(8,3,10,0.97)" />
      {[0,20,40,60,80].map(x => (
        <rect key={`frw${x}`} x={1007+x} y="143" width="12" height="14" fill="rgba(8,3,10,0.97)" />
      ))}

      {/* ── Tower 10: Far-far right minor tower ── */}
      ...
      {[0,18,36].map(x => (
        <rect key={`ew${x}`} x={1147+x} y="148" width="12" height="14" fill="rgba(8,3,10,0.97)" />
      ))}
...
// Vine/creeper border SVG — deep purple strokes
function CreeperVine({ flip = false }) {
  return (
    ...
    >
      <path
        d="M0,20 C30,5 50,35 80,20 C110,5 130,35 160,20 C190,5 210,35 240,20 C270,5 290,35 320,20 C350,5 370,35 400,20 C430,5 450,35 480,20 C510,5 530,35 560,20 C590,5 610,35 640,20 C670,5 690,35 720,20 C750,5 770,35 800,20"
        stroke="rgba(80,30,160,0.5)"
        strokeWidth="1.5"
        fill="none"
        strokeDasharray="800"
        strokeDashoffset="0"
        style={{ animation: 'vine-grow 3s ease-out both' }}
      />
      {/* Leaf nodes */}
      {[40,120,200,280,360,440,520,600,680,760].map((x, i) => (
        <g key={x} transform={`translate(${x}, ${i % 2 === 0 ? 12 : 28})`}>
          <ellipse cx="0" cy="0" rx="5" ry="3" fill="rgba(60,20,100,0.45)" transform="rotate(30)" />
        </g>
      ))}
      {/* Thorns */}
      {[80,160,240,320,400,480,560,640,720].map((x, i) => (
        <path
          key={x}
          d={`M${x},20 L${x + (i%2===0?4:-4)},${i%2===0?14:26}`}
          stroke="rgba(100,40,180,0.4)"
          strokeWidth="1"
          fill="none"
        />
      ))}
...
function HomePage() {
  return (
    <main className="homepage">
      ...
      <section className="hero section" id="home">
        <div className="hero-frame">
          <figure className="hero-media">
            <img
              src="/hero.png"
              alt="Vindom hero artwork showing a dark figure before a gothic city under a red moon"
          ...
          <div className="hero-copy">
            <p className="eyebrow hero-eyebrow">Hear the Truth of Nature</p>
            <h1 className="hero-story-title">All things dwell between what endures and what is changed.</h1>
            ...
            <div className="hero-pillars" aria-label="The two powers of Vindom">
            ...
            <div className="hero-actions hero-actions-left">
              ...
              <Link to="/cosmology" className="button button-secondary">
                Learn the Cosmology
              </Link>
            </div>
          </div>
      ...
      {/* ── THE TWO DARK POWERS ─────────────────────── */}
      <section className="section" id="divine-powers" style={{ position: 'relative' }}>
        ...
        <div className="deity-grid">
          <article className="deity-card vin-card">
            ...
            <div className="deity-content">
              <h3>
                <span style={{ color: 'rgba(139,92,246,0.7)', marginRight: '0.4rem' }}>&#9760;</span>
                Vin &mdash; Goddess of Transformation
              </h3>
              <p>Vin is the power by which all forms are changed.</p>
              <p>
                She is present in endings, in growth, in sorrow, in healing, and
                in every hour when life asks us to loosen one form so another may appear.
              </p>
              <p>
                What seems like ruin at close hand is often, in Vindom,
                the universe preparing room for a deeper life.
              </p>
              <p>Through Vin, the cosmos does not lie still.</p>
              <p>
                <strong style={{ color: 'rgba(160,110,255,0.9)' }}>It continues to become.</strong>
              </p>
            </div>
          </article>
          ...
          <article className="deity-card ken-card">
            ...
            <div className="deity-content">
              <h3>
                <span style={{ color: 'rgba(139,92,246,0.7)', marginRight: '0.4rem' }}>&#10022;</span>
                Ken &mdash; Keeper of Order
              </h3>
              <p>Ken is the keeper of foundations.</p>
              <p>
                He is present in vows kept, homes built with care, habits that
                guard us, and every structure that helps life endure.
              </p>
              <p>
                Through Ken arise the patterns by which the world holds
                together: law, memory, discipline, and shared order.
              </p>
              <p>Without Ken, nothing would remain steady long enough to grow.</p>
              <p>
                <strong style={{ color: 'var(--ivory)' }}>He gives life a firm place upon which to stand.</strong>
              </p>
            </div>
          </article>
        </div>
      ...
      <section className="section" id="doctrine">
        ...
        <p className="lead" style={{ margin: '0 0 0.8rem' }}>
          Vindom teaches that life is best understood when we behold both
          sides of nature: what must be built, and what must be changed.
        </p>
        ...
        >
          <div
            style={{
              padding: '1.2rem',
              border: '1px solid rgba(80,30,160,0.5)',
          ...
          >
            ...
            <p style={{ margin: 0, color: 'var(--bone-dark)', fontSize: '0.9rem' }}>
              Structure, law, pattern, and endurance
            </p>
          </div>
          <div
            style={{
              padding: '1.2rem',
              border: '1px solid rgba(100,40,200,0.5)',
              background: 'rgba(25,5,50,0.6)',
              textAlign: 'center',
            }}
          >
            <div style={{
            ...
            <p style={{ margin: 0, color: 'var(--bone-dark)', fontSize: '0.9rem' }}>
              Change, entropy, and renewal
            </p>
          </div>
        </div>

        <p className="lead" style={{ margin: '1rem auto 0.5rem' }}>
          If all things remained the same forever, life would harden into stillness.
          If all things changed without shape or care, life would fall into disorder.
        </p>
        <p className="lead">
          Vindom dwells in the balance between these truths: steady enough to endure,
          open enough to grow.
        </p>
        <p className="lead">
          <strong>To feel that rhythm within your own life is the beginning of wisdom.</strong>
        </p>
      </section>
      ...
      <section className="section" id="hymns">
        ...
        <p className="lead" style={{ margin: '0 0 0.6rem' }}>
          ...
          <strong style={{ color: 'rgba(160,110,255,0.9)' }}>Hymns of Vin</strong>
          {' '}and the{' '}
        ...
        <p className="lead">Through them, the teachings of Vindom become easier to feel and understand.</p>
      ...
      <section className="section" id="path">
        ...
        <p className="lead" style={{ margin: '0 0 0.7rem' }}>
          In the common life of mortals, Vindom is not about choosing order over change.
          It is about learning when one is needed, when the other is called for, and how to live with both.
        </p>
